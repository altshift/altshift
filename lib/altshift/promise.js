...
var Promise = core.class('Promise', {
    ...
    then: function (resolvedCallback, errorCallback, progressCallback) {
        throw new core.NotImplemented("The Promise base class is abstract, this function must be implemented by the Promise implementation");
    },
...
var Deferred = core.class('Deferred', Promise, {
    ...
    initialize : function (canceller) {
        ...
        currentContextHandler = contextHandler.getHandler && contextHandler.getHandler();
        ...
        function notifyAll(value) {
            ...
            var i, length;
            for (i = 0, length = waiting.length; i < length; i += 1) {
                notify(waiting[i]);
            }
        }

        // calling resolve will resolve the promise
        this.emitSuccess = function (value) {
            notifyAll(value);
        };

        // calling error will indicate that the promise failed
        this.emitError = function (error, dontThrow) {
            isError = true;
            notifyAll(error);
            if (!dontThrow) {
                enqueue(function () {
        ...
        // call progress to provide updates on the progress on the completion of
        // the promise
        this.progress = function (update) {
            var i, length, progress;
            for (i = 0, length = waiting.length; i < length; i += 1) {
                progress = waiting[i].progress;
                if (progress) {
                    progress(update);
                }
            }
