...
var Deferred = core.class('Deferred', Promise, {
     ...
     */
    then: function (resolvedCallback, errorCallback, progressCallback) {
        var returnDeferred = new Deferred(this.promise.cancel),
            listener = {
    ...
    timeout: function (milliseconds) {
        ...
        this._timer = setTimeout(function () {
            if (!self.finished) {
                if (self.promise.cancel) {
                    self.promise.cancel(new core.Error({code: 'timeout'}));
                } else {
                    self.emitError(new core.Error({code: 'timeout'}));
                }
            }
        }, milliseconds);
        return this.promise;
