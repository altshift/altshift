...
var Client = core.class('Client', {
     ...
     * @return {Promise}
     */
    request : function (originalRequest, callback) {
        var request = {},
            httpClient,
            httpRequest,
        ...
        if (request.url) {
            request.url = (request.url instanceof Uri) ? request.url : new Uri(request.url);
            url = request.url;
        ...
        requestPath = (url.pathname || url.pathInfo || '');
