...
var ClassExceptionTest = core.class('ClassExceptionTest', core.BaseException);

function createClass(name, classParent, methods) {
    return core.class(name, classParent, methods);
}

function createInterface(name, methods) {
    return core.interface(name, methods);
}
...
var ClassTest = vows.describe('Class class').addBatch({
    "Class()" : {
        ...
        'should throw error if name is empty' : function (topic) {
